...
final class ViewerDeviceSession: ViewerAdmissionSessionReceiving, @unchecked Sendable {
  ...
  private func receiveActive(
    ...
    case .eventDropSummary:
      ...
      let first = Self.saturatingAdd(summary.overflowDropped, summary.expired)
      remoteDroppedEvents = Self.saturatingAdd(
        remoteDroppedEvents,
        Self.saturatingAdd(first, summary.coalesced)
      )
      return .consume
    case .ping:
      guard turnSystemCount < 32 else { return .pause }
  ...
  private func admitIncoming(_ records: [WireEventRecord], receipt: UInt64) throws {
    ...
    for record in records {
      ...
      let pending = try PendingEvent(
        ...
        accountedByteCount: try record.deterministicEncodedByteCount(),
  ...
  private func drainDownlink(now: UInt64) {
    ...
    do {
      ...
      for pending in batch.events {
        ...
        records.append(try WireEventRecord(envelope: envelope, nowOnOriginClockNanoseconds: now))
      }
      let data: Data
      if records.count == 1 {
...
private final class ViewerUplinkHandoff: @unchecked Sendable {
  ...
  func offer(
    ...
    let payload = ViewerUplinkPayload(event)
    let operation = BlockOperation { [weak self, payload] in
      guard let event = payload.take() else { return }
...
private final class ViewerUplinkPayload: @unchecked Sendable {
  private let lock = NSLock()
  private var event: WireReceivedEvent?

  init(_ event: WireReceivedEvent) { self.event = event }

  func take() -> WireReceivedEvent? {
    lock.lock()
