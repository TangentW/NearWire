...
Implementation refinement: pairing refresh prepares and exactly registers the replacement publication before cancelling the old listener. A replacement failure therefore preserves the usable code, while a successful commit cancels only unhanded old-generation admissions and leaves active sessions intact.
...
- Maintained Viewer updates use one stable Apple Development signer for internal builds or one stable Developer ID signer for distribution. Ad-hoc signatures are limited to isolated tests and do not support the cross-update login-Keychain identity contract.
- 刷新对频码不重新生成证书；证书只在 Viewer 重装、Keychain 被清除或用户执行“重置 TLS 身份”时变化。
- Automatic rotation is also permitted when the owned certificate is invalid or has less than 30 days remaining. Viewer stops publication first, performs one fail-closed renewal attempt, and never treats pairing-code refresh as certificate rotation.
...
- Run an update-boundary integration test with two independently built products signed by the same supported signer; prove exact identity reuse, private-key signing use, both reset scopes, and denial to an unrelated signer.
...
- The maintained Viewer project defaults to automatic Apple Development signing. Each internal team selects its stable development team once; Developer ID is the supported distributed-update alternative.
