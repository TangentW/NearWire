...
The native macOS listener, persistent Viewer identity, pairing-code publication, bounded new-device admission, sandbox, and recovery behavior are documented in [Documentation/Viewer-Foundation.md](Documentation/Viewer-Foundation.md). The process-scoped Viewer Session, top Devices strip, Event inspection, filtering, Clear, complete-Session JSON import/export, panel controls, and Viewer-to-App control composition are documented in [Documentation/Viewer-Event-Explorer.md](Documentation/Viewer-Event-Explorer.md). The single-device performance analysis surface, time and availability semantics, projection bounds, raw traceability, and privacy behavior are documented in [Documentation/Viewer-Performance.md](Documentation/Viewer-Performance.md).
