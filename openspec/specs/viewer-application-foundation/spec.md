...
The application SHALL expose one main window and no supported second-listener window. Opening the window SHALL start one runtime generation without a Start button. Closing the last window or terminating the application SHALL synchronously close admission, stop publication/listening, cancel pending attempts, and await one idempotent cleanup receipt for at most one second without leaving a hidden listener. Expiry of that wait SHALL NOT reopen admission or abandon cleanup ownership.

#### Scenario: Main window opens and closes

- **WHEN** the NearWire main window opens and its runtime dependencies succeed
- **THEN** exactly one Viewer runtime generation starts automatically
- **AND** closing the last window stops that exact generation without a menu-bar or daemon lifetime
...
- **WHEN** application termination or identity reset closes a runtime whose owned handoff cleanup does not complete within one second
- **THEN** the bounded wait returns without reopening listener admission
- **AND** the same cleanup owner remains responsible for eventual completion
...
The main window SHALL show pairing/listener status, Copy, Refresh, Pause/Resume, the approval setting, pending approval actions, fixed identity/listener recovery actions, and a later-workspace placeholder. It SHALL label transport as `TLS encrypted; Viewer identity is not authenticated` and SHALL state that the pairing code and stable `vid` are visible to nearby Bonjour browsers. It SHALL NOT call either value a password, secret, authentication token, or secure connection proof.

All controls SHALL expose accessibility labels and disabled states derived from the single application model. User-visible and diagnostic errors SHALL use closed safe categories and SHALL NOT include pairing code, Keychain labels, identity values, certificate data, endpoint/interface descriptions, wire bytes, App content, or arbitrary system error text.

#### Scenario: Listener is ready

- **WHEN** the exact service is registered
- **THEN** pairing actions become enabled and the truthful TLS limitation remains visible

#### Scenario: Runtime enters a safe failure

- **WHEN** identity, listener, registration, or admission startup fails
...
Viewer SHALL enable App Sandbox with only the incoming network-server entitlement needed by this foundation. It SHALL NOT request network-client, multicast, Keychain-sharing, application-group, or background-service entitlements. The built Info.plist SHALL list `_nearwire._tcp` in `NSBonjourServices` and SHALL contain the English local-network usage description `NearWire advertises a local service so your iPhone apps can connect to this Mac.` Local-network denial SHALL produce a fixed recoverable failure and no alternate or plaintext listener.

Viewer SHALL package its own valid `PrivacyInfo.xcprivacy` declaring linked Device ID for App functionality and tracking false because it publishes stable `vid` and sends its installation ID in Viewer Hello. It SHALL omit tracking domains and SHALL declare `NSPrivacyAccessedAPICategoryUserDefaults` with reason `CA92.1` for the app-local approval preference while omitting unused Required Reason API categories. Build evidence SHALL inspect the final Info.plist, signed/ad-hoc entitlements, and packaged privacy resource.

#### Scenario: Built Viewer metadata is audited

- **WHEN** the committed Viewer application is built for macOS
- **THEN** its product contains the local-network description, NearWire Bonjour service, exact sandbox server entitlement, and Viewer privacy manifest
- **AND** no client, multicast, Keychain-sharing, app-group, tracking, or unused Required Reason declaration is present beyond the required app-local UserDefaults reason
