...
The native macOS page SHALL use scalable current cards, a fixed 16-key availability section, six
bounded system Charts views, fixed ranges, one synchronized pointer/keyboard crosshair, aggregate
tooltip, representative raw action, and deterministic English empty/loading/live-only/unavailable/
error guidance. State SHALL not rely on color. Accessibility SHALL combine metric, unit, Viewer time,
statistics, discontinuity, and availability within the 64-summary-per-chart cap.

Received values SHALL have no copy, cut, drag, share, clipboard-export, preference, restoration,
recent-row, safe-status-row, log, analytics, or content-bearing reflection sink. Mode switch SHALL
clear crosshair/tooltip and active work. Runtime end, window close, listener failure, TLS/full reset,
Store/source/device replacement, deinitialization, and claimed-delivery cleanup SHALL cancel/join
projection/reveal/deadline work and clear cards, buckets, categorical state, diagnostics, decoded
summaries, live slice, cache, locators, tooltip, accessibility values, and delivery state before the
existing receipt completes. Unsealed controller deinitialization SHALL synchronously seal any
externally retained model and SHALL transfer its cancellation/join receipts and required owners to a
detached cleanup owner until all work and charged memory reach zero.

#### Scenario: Source changes while paused

- **WHEN** a different device/source is selected while old performance presentation is paused
- **THEN** old content and cache clear immediately and no old raw action remains available
- **AND** Pause cannot display prior-device values under the successor scope

#### Scenario: Claimed chart delivery races cleanup

- **WHEN** cleanup begins after a chart result claimed MainActor delivery
- **THEN** cleanup waits until that exact result is discarded and every performance value is cleared
- **AND** a replacement runtime contains no prior metric, tooltip, accessibility value, or locator
