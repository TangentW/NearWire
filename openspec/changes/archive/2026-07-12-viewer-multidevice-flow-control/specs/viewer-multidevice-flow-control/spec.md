...
Every new error, terminal category, description, debug description, reflection helper, interpolation, and log SHALL derive only from a closed local code. It SHALL exclude Event type/content, metadata values, queue keys, installation/correlation/Bundle identifiers, nicknames, rates, queue values, session epochs, endpoints, certificate data, peer text, raw bytes, and underlying errors. Event drafts, encoded payloads, queue keys, session epochs, and queue contents SHALL be absent from `UserDefaults`, logs, analytics, clipboard, exported data, UI state, and recent rows. Effective policy SHALL be absent from persistence, logs, analytics, clipboard, exported data, and recent rows but MAY appear in the bounded safe session snapshot. Packaging evidence SHALL document whether the existing Viewer Device ID collection and UserDefaults required-reason declarations remain sufficient for bounded correlation/nickname preferences and SHALL inspect the built privacy manifest.
...
- **THEN** remote-loss telemetry increases with saturation
